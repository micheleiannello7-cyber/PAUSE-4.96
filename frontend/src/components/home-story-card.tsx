...
export function HomeStoryCard({ story, active, onOpen, onListen }: {
  story: StoryPreview; active: boolean; onOpen: () => void; onListen?: () => void;
}) {
  ...
  return (
    <View style={styles.card} testID={active ? `story-card-${story.id}` : `deck-card-${story.id}`}>
      ...
      />
      <Pressable testID={`${id}-open`} style={StyleSheet.absoluteFill} onPress={onOpen}
        accessibilityRole="button" accessibilityLabel={`${t.read_story}: ${story.title}`} disabled={!active} />
      ...
      <View style={styles.body} pointerEvents="box-none">
        ...
        <View style={styles.cta}>
          <FrostedButton label={t.read_story} icon="book-outline" onPress={onOpen} overImage
            disabled={!active} style={styles.readButton} testID={active ? "approfondisci-home" : `${id}-cta`} />
          {onListen ? <FrostedButton label={t.audio_listen_short} icon="headset-outline" onPress={onListen} overImage
            disabled={!active} style={styles.listenButton} testID={active ? "home-listen-story" : `${id}-listen`} /> : null}
        </View>
      </View>
    </View>
  );
...
const useStyles = makeStyles((colors) => ({
  ...
  heading: { gap: 12 },
  cta: { marginTop: 4, flexDirection: "row", gap: 10 },
  readButton: { flex: 1.6 },
  listenButton: { flex: 1 },
}));